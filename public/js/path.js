...
function drawBoard() {
  for (let i = 1; i <= 375; i++) {
    ...
    box.style = `background:#17a2b8;height:15 px;width:15 px;`;
    box.style.backgroundSize = "15px 15px";
  ...
  sourceDiv.style.backgroundImage =
    "url('images/" + Theme.Source + "')";

  wallDiv.style.backgroundImage = "url('images/" + Theme.Wall + "')";

  goalDiv.style.backgroundImage = "url('images/" + Theme.Goal + "')";

  pathDiv.style.backgroundImage = "url('images/" + Theme.Path + "')";

  let sBox = document.querySelector(`.box${source}`);
  let dBox = document.querySelector(`.box${dest}`);
  sBox.style.backgroundImage = "url('images/" + Theme.Source + "')";
  dBox.style.backgroundImage = "url('images/" + Theme.Goal + "')";
...
findbtn.addEventListener("click", async function findPath() {
  ...
  nodes.forEach((item) => {
    if (item.style.backgroundImage == `url("images/${Theme.Wall}")`) {
      item.style.background = "#17a2b8"; //original
      item.style.backgroundSize = "25px 25px";
      item.style.backgroundImage = "url('images/" + Theme.Wall + "')";
    } else if (
      item.classList[0].slice(3) == source ||
      item.classList[0].slice(3) == dest
    );
  ...
  while (!path.isEmpty()) {
    ...
    pathBox.style.backgroundImage = "url('images/" + Theme.Path + "')";
...
async function drawWallPreset(wallPreset) {
  ...
  for (let i of wallPreset) {
    ...
    if (
      ...
      iBox.style.backgroundImage = "url('images/" + Theme.Wall + "')";
      for (let i = 0; i <= 25; i += 5) {
